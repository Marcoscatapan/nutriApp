...
def adicionar_botao_voltar():
    ...
    with col2:
        if st.button("🏠 Voltar para Página Inicial", use_container_width=True):
            # Limpar session state se necessário
            keys_to_clear = [k for k in st.session_state.keys() if k not in ['pagina_atual']]
            for key in keys_to_clear:
                del st.session_state[key]
            st.session_state.pagina_atual = "inicio"
            st.rerun()

# No final da função main() de cada arquivo, antes do if __name__,
# adicione a chamada para o botão de voltar:
