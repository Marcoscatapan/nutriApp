...
with st.sidebar:
    ...
    # Links de navegação usando st.page_link
    st.page_link("app.py", label="🥗 Página Inicial", icon="🏠")
    st.markdown("---")
    st.markdown("### 🎯 Sistemas")
    st.page_link("pages/nutricao_convencional.py", label="🥗 Nutrição Convencional", icon="📊")
    st.page_link("pages/nutricao_triatlo.py", label="🏊‍♂️ Nutrição Triatlo", icon="⚡")
    
    st.markdown("---")
    st.markdown("### 💡 Ajuda")
    st.markdown("Selecione o sistema acima conforme sua necessidade:")
    st.markdown("- **🥗 Nutrição Convencional**: Pacientes em geral")
    st.markdown("- **🏊‍♂️ Nutrição Triatlo**: Atletas de endurance")
